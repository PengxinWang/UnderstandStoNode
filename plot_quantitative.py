...
def process_data(results):
    ...
    for model_name, data in results.items():
        ...
        nll_data = {intensity: [] for intensity in intensities}  # NLL data

        for entry in model_data:
            intensity = entry['intensity']
            acc_data[intensity].append(entry['acc'])
            ece_data[intensity].append(entry['ece'])
            nll_data[intensity].append(entry['nll'])  # Extract NLL
        ...
        processed_data[model_name] = {
            ...
            'nlls': [sum(nll_data[i])/len(nll_data[i]) for i in intensities],  # Average NLL
        }
